...
const BannerEditor = () => {
    ...
    const loadBanner = async () => {
      ...
      try {
        ...
        if (snap.exists()) {
          ...
          setFormError("Bannière introuvable.");
        }
      } catch (err) {
        console.error("Failed to load banner:", err);
        setFormError("Impossible de charger la bannière.");
      } finally {
  ...
  const canSubmit = useMemo(() => {
    if (loadingBanner || submitting) return false;
  ...
  const handleSubmit = async (event) => {
    ...
    if (!subtitle.trim()) {
      setFormError("Renseignez le sous-titre.");
      return;
    }
    if (!badge.trim()) {
    ...
    try {
      ...
      if (heroImageFile) {
        const storageRef = ref(
          storage,
          `banners/${Date.now()}-${heroImageFile.name.replace(/\s+/g, "_")}`
        );
      ...
      if (isEdit) {
        ...
        setFormSuccess("Bannière mise à jour.");
      } else {
      ...
      <div className="marketingPage__container">
        ...
        <div className="marketingPage__content">
          <div className="marketingPage__header marketingPage__header--between">
            <div>
              <h1>{isEdit ? "Modifier la bannière" : "Créer une bannière"}</h1>
              <p className="subtitle">Données enregistrées dans la collection banners.</p>
            </div>
            <Link to="/admin/marketing/banners" className="ghost">
              Retour à la liste
            </Link>
          </div>

          <form className="marketingPage__form" onSubmit={handleSubmit}>
            {loadingBanner && <p className="helper">Chargement...</p>}
            ...
            <div className="formRow">
              <label>
                Sous-titre <span className="requiredStar">*</span>
              </label>
              <input
                type="text"
                value={subtitle}
